...
app.get("/",function(req,res){
  ...
  res.redirect("/display",req.session)}
})
...
//	var kudoscount=req.query.kudoscount
	//kudoscount=kudoscount+1
	//this.value='Kud'
	//var datatopass={data:docs,username:req.session.username,kudoscount:req.query.kudoscount}
...
app.get("/delete",function(req,res){
  ...
  // numRemoved = 1
	//var datatopass = {data:numRemoved,username: req.session.username};
...
app.get("/individual", function(req,res){
  ...
  //  res.send(docs);
    ...
    var datatopass = {data:docs,username: req.session.username};
...
app.get('/search', function(req,res) {
    var searchters = req.query.search;
    var searchregex = new RegExp(searchters);
		var searchtaipu =req.query.searchtype
		var searchtaipuregex = new RegExp(searchtaipu);


		if (searchtaipu == 'title'){
			db.find({title:searchregex}).sort({timestamp:1}).exec(function (err,docs){
				console.log(docs);
				var datatopass = {data:docs, category:"Search " + searchters};
				res.render("display.ejs", datatopass);
			});

		}else if (searchtaipu == 'authorname'){
			db.find({authorname:searchregex}).sort({timestamp:1}).exec(function (err,docs){
				console.log(docs);
				var datatopass = {data:docs, category:"Search " + searchters};
				res.render("display.ejs", datatopass);
			});

		}else if (searchtaipu == 'posttype'){
			db.find({posttype:searchregex}).sort({timestamp:1}).exec(function (err,docs){
				console.log(docs);
